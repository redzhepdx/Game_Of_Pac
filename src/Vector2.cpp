...
template int Vector2<int>::magnitute() const;

template <typename T>
Vector2<T> Vector2<T>::normalize(){
	T magnitute = this->magnitute();
	
	float inv_sqrt_mag = inverse_sqrt<float, 2>((float)magnitute);
...
template Vector2<int> Vector2<int>::operator*(const int value) const;
