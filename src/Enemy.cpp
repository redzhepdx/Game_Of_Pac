...
Enemy::Enemy(uint32_t textureBufferID, Vector2<float> position) : Sprite(textureBufferID, position)
{
	m_Type = Chaser;
...
Enemy::Enemy(uint32_t textureBufferID, Vector2<float> position, EnemyType type) : Sprite(textureBufferID, position)
{
	m_Type = type;
