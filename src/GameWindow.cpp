...
void GameWindow::setupGL()
{
    ...
    m_MaxTileCount = static_cast<int32_t>(((m_Width * m_Height) / (SQUARE_SIZE * SQUARE_SIZE)) * m_TilePercent);
}
...
void GameWindow::setupArena()
{
    ...
    m_Maze = std::make_unique<Maze>(static_cast<int32_t>(m_Height / SQUARE_SIZE),
                                    static_cast<int32_t>(m_Width / SQUARE_SIZE));
...
void GameWindow::generateRandomMap()
{
    ...
    int32_t max_row = static_cast<int32_t>(m_Width / SQUARE_SIZE);
    int32_t max_col = static_cast<int32_t>(m_Height / SQUARE_SIZE) - 1;

    //First Random Tile
    Vector2<int32_t> firstRandomPos(arc4random() % max_row + min_row,
...
void GameWindow::generateExitPoints()
{
    ...
    int32_t vertical_teleport_pos = static_cast<int32_t>(dist_col(gen));
    int32_t horizontal_teleport_pos = static_cast<int32_t>(dist_row(gen));
    ...
    for (size_t close_tile_idx = 0; close_tile_idx < exit_tiles.size(); ++close_tile_idx)
    {
        Vector2<int32_t> tile = exit_tiles[close_tile_idx];
        std::vector<float> distances;

        // In-efficient asF
        std::for_each(m_EmptyTiles.begin(), m_EmptyTiles.end(),
                      [&](Vector2<int32_t> selected_tile)
                      {
    ...
    for (size_t close_tile_idx = 0; close_tile_idx < close_tiles.size(); ++close_tile_idx)
    {
        ...
        if ((size_t)close_tile_idx >= (exit_tiles.size() / 2))
        {
            ...
            if (std::abs(col_difference) > 0)
            {
                ...
                {
                    (col_difference > 0) ? m_Maze->m_Matrix[exit_tiles[close_tile_idx].x][exit_tiles[close_tile_idx].y - step] = Empty : m_Maze->m_Matrix[exit_tiles[close_tile_idx].x][exit_tiles[close_tile_idx].y + step] = Empty;
                }
            }
            ...
            if (std::abs(row_difference) > 0)
            {
                ...
                {
                    (row_difference > 0) ? m_Maze->m_Matrix[close_tiles[close_tile_idx].x + step][close_tiles[close_tile_idx].y] = Empty : m_Maze->m_Matrix[close_tiles[close_tile_idx].x - step][close_tiles[close_tile_idx].y] = Empty;
                }
            }
        }
        else
        {
            ...
            if (std::abs(row_difference) > 0)
            {
                for (auto step = 1; step <= std::abs(row_difference); ++step)
                {
                    (row_difference > 0) ? m_Maze->m_Matrix[exit_tiles[close_tile_idx].x - step][exit_tiles[close_tile_idx].y] = Empty : m_Maze->m_Matrix[exit_tiles[close_tile_idx].x + step][exit_tiles[close_tile_idx].y] = Empty;
                }
            }
            ...
            if (std::abs(col_difference) > 0)
            {
                for (auto step = 1; step <= std::abs(col_difference); ++step)
                {
                    (col_difference > 0) ? m_Maze->m_Matrix[close_tiles[close_tile_idx].x][close_tiles[close_tile_idx].y + step] = Empty : m_Maze->m_Matrix[close_tiles[close_tile_idx].x][close_tiles[close_tile_idx].y - step] = Empty;
                }
            }
        }
    }
    ...
    // Last
    for (int32_t row = 0; row < static_cast<int32_t>(m_Height / SQUARE_SIZE) + 1; ++row)
    {
        for (int32_t col = 0; col < static_cast<int32_t>(m_Width / SQUARE_SIZE) + 1; ++col)
        {
...
void GameWindow::initPlayer()
{
    ...
    if (m_Player.get() == nullptr)
    {
        ...
        m_Player = std::make_unique<Player>(m_TextureBufferID, Vector2<float>(randomPosition.y * SQUARE_SIZE, m_Width - randomPosition.x * SQUARE_SIZE),
...
bool GameWindow::isPositionValid(Vector2<int32_t> &position, int32_t min_row, int32_t max_row, int32_t min_col, int32_t max_col)
{
...
BinaryImage GameWindow::getCurrentImageGameState(GLFWwindow *window)
{
    ...
    buffer = std::vector<u_char>(outputTensor.data_ptr<uint8_t>(), outputTensor.data_ptr<uint8_t>() + outputTensor.numel());
