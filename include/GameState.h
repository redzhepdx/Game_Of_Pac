...
public:
    GameStateInternal(Vector2<float> player_pos,
        ...
        m_AvailableProjectiles     = available_projectiles;
        m_TimeToTeleport           = time_to_teleport;
        m_PlayerHealth             = player_health;
        m_NormalTilePositions      = normal_tile_positions;
        m_TeleportTilePositions    = teleport_tile_positions;
        m_EnemyPositions           = enemy_positions;
        m_ActiveBulletPositions    = bullet_positions;
        m_ActiveBulletDirections   = bullet_directions;
    }
...
private:
    u_int16_t                          m_Width;
    u_int16_t                          m_Height;
    u_int16_t                          m_Channels;
    std::vector<BinaryImage>           m_StateImages;
    InterpolateFuncOptions             m_ResizeOptions = InterpolateFuncOptions().size(std::vector<int64_t>({m_Width / 8, m_Height / 8})).mode(torch::kBilinear);
public:
    GameStateImage(std::vector<BinaryImage> images, u_int16_t width, u_int16_t height, u_int16_t channels) : 
                                                        m_StateImages(images), m_Width(width), m_Height(height), m_Channels(channels){}
