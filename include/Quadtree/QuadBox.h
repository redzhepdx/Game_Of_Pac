...
namespace quadtree {
    ...
    public:
        ...
        constexpr QuadBox(T Left = 0, T Top = 0, T Width = 0, T Height = 0) noexcept: left(Left), top(Top),
                                                                                      width(Width), height(Height) {
        }

        constexpr QuadBox(const QuadVector2<T> &position, const QuadVector2<T> &size) noexcept: left(position.x),
                                                                                                top(position.y),
                                                                                                width(size.x),
                                                                                                height(size.y) {
        }
