...
namespace quadtree {
    ...
    public:
        Quadtree(const QuadBox<Float> &box, const GetBox &getBox = GetBox(),
                 const Equal &equal = Equal()) : mBox(box), mRoot(std::make_unique<Node>()), mGetBox(getBox),
                                                 mEqual(equal) {
        }
    ...
    private:
        ...
        QuadBox<Float> computeBox(const QuadBox<Float> &box, int i) const {
            ...
            switch (i) {
                ...
                case 0:
                    ...
                    // Norst East
                case 1:
                    return QuadBox<Float>(Vector2<Float>(origin.x + childSize.x, origin.y), childSize);
                    // South West
                case 2:
                    return QuadBox<Float>(Vector2<Float>(origin.x, origin.y + childSize.y), childSize);
                    // South East
                case 3:
        ...
        void add(Node *node, std::size_t depth, const QuadBox<Float> &box, const T &value) {
            assert(node != nullptr);
        ...
        void split(Node *node, const QuadBox<Float> &box) {
            assert(node != nullptr);
        ...
        void remove(Node *node, Node *parent, const QuadBox<Float> &box, const T &value) {
            assert(node != nullptr);
            assert(box.contains(mGetBox(value)));
            if (isLeaf(node)) {
        ...
        void
        query(Node *node, const QuadBox<Float> &box, const QuadBox<Float> &queryBox, std::vector<T> &values) const {
            assert(node != nullptr);
